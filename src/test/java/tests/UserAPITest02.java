...
public class UserAPITest02 extends BaseTest01 {
	...
	public void userAPITests() throws Exception {
	    ...
	    for (Map<String, Object> testCase : testCases) {
	        ...
	        RequestSpecification requestSpec = BaseTest01.getRequestWithBasicAuth();
	        ...
	        if (expectedStatusCode == 201) {
	            ...
	            // Negative scenario: Validate error message and status text
	            String errorMessage = response.jsonPath().getString("message");
	            Assert.assertNotNull(errorMessage, "Expected 'message' field in error response");

	            String expectedErrorMessage = (String) testCase.get("expectedErrorMessage");
	            if (expectedErrorMessage != null) {
	                Assert.assertEquals(errorMessage, expectedErrorMessage, "Error message mismatch");
	            }
	            
	            String actualStatusText = response.jsonPath().getString("status");
	            String expectedStatusText = (String) testCase.get("expectedStatusText");
	            if (expectedStatusText != null) {
	                Assert.assertEquals(actualStatusText, expectedStatusText, "Status text mismatch");
	            }
	        }
	    }
